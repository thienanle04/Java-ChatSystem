    ...
    public List<Model_Group_Chat> getChatListId(int userId) throws SQLException {
        ...
        while (r.next()) {
            ...
            if (type == GroupType.TWO) {
                ...
                PreparedStatement p2 = con.prepareStatement("select u.username, u.status from group_members gm join users u on u.user_id = gm.user_id where gm.group_id = ? and gm.user_id <> ?");
                p2.setInt(1, groupID);
                p2.setInt(2, userId);
